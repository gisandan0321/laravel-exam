<template>
  <section>
    <form class="lg:w-2/3" @submit.prevent="handleSubmit" novalidate>
      <h1 class="block text-gray-700 text-xl">Leave a Comment</h1>
      <label for="name" class="block text-md text-gray-600 mt-4">
        Name
      ...
      <label for="message" class="block text-md text-gray-600 mt-4">
        Comment
        <span class="text-red-500">*</span>
      </label>
      <textarea
      ...
      <button
        type="button"
        class="block w-1/5 bg-teal-500 p-2 text-white text-sm rounded cursor-not-allowed mt-3"
        v-if="isProcessing"
      >
